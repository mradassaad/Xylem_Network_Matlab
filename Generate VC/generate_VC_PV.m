function [PLC,PV,Pressures,Kmax,Kini,embNb,ncNb] = ...
    generate_VC_PV(sim,jt,stepsize,itmax,inEmb, ICLogical)
K=zeros(jt,itmax+1);
...
%     while i<=itmax && K(j,i-1)/K(j,1)>0.05 && flag<5
    while i<=itmax && K(j,i-1)/K(j,1)>0.1
        ...
        if Kcurrent~=-1
            ...
            PV(j,i) = sum([embcond.Length].*[embcond.Diameter].^2.*pi./4);
        else
            K(j,i)=K(j,i-1);
            embNb(j,i)=sum([sim.Conduits.Embolized]);
            ncNb(j,i)=sum([sim.Conduits.NonConducting]);
...
%         end
    ...
    PV(j,find(PV(j,:)~=0,1,'last'):end) = PV(j,find(PV(j,:)~=0,1,'last'));
    j=j+1;
end
